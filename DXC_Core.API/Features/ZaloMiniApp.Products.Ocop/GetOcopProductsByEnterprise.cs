...
public static class GetOcopProductsByEnterprise
{
    public class Query : IRequest<PagedResult<OcopProductDto>>
    {
        public Guid EnterprisePublicId { get; set; } = Guid.Empty;
    ...
    public class Validator : AbstractValidator<Query>
    {
        public Validator()
        {
            RuleFor(x => x.EnterprisePublicId).NotEqual(Guid.Empty).WithMessage("EnterprisePublicId là bắt buộc");
    ...
    public class Handler(ZaloMiniAppDbContext dbContext) : IRequestHandler<Query, PagedResult<OcopProductDto>>
    {
        public async Task<PagedResult<OcopProductDto>> Handle(Query request, CancellationToken cancellationToken)
        {
            var query = dbContext.OcopProducts
                ...
                .Where(p => p.IsActive && p.Enterprise.IsActive)
                .Where(p => p.Enterprise.PublicId == request.EnterprisePublicId);
            ...
            return new PagedResult<OcopProductDto>
            {
                ...
                Message = $"Lấy thành công {products.Count} sản phẩm của doanh nghiệp"
            };
        }
    }
}