...
public static class UpdateOcopProduct
{
    ...
    public class Validator : AbstractValidator<Command>
    {
        ...
        private async Task<bool> BeUniqueName(Command command, string name, CancellationToken cancellationToken)
        {
            return !await _zaloContext.OcopProducts
                .AnyAsync(p => p.Name == name && p.PublicId != command.PublicId, cancellationToken);
        }
    ...
    public class Handler : IRequestHandler<Command, ApiResult<OcopProductDto>>
    {
        ...
        {
            var product = await _zaloContext.OcopProducts
                .Include(p => p.Images)
            ...
            product.EnterpriseId = enterprise!.Id;
            product.ReferencePrice = request.ReferencePrice;
            product.PromotionalPrice = request.PromotionalPrice;
            product.ContactPhone = request.ContactPhone;
            product.ContactAddress = request.ContactAddress;
            ...
            if (request.ImagePublicIds != null)
            {
                ...
                _zaloContext.OcopProductImages.RemoveRange(product.Images);
            ...
            return new ApiResult<OcopProductDto>
            {
                Success = true,
                Data = productDto,
                Message = "Cập nhật sản phẩm OCOP thành công"
            };
        }
    }
}
