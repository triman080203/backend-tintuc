...
namespace DXC_Core.API.Features.ZaloMiniApp.Services.IconManagement;
    ...
    public static class UpdateIconGroup
    {
        ...
        public class Validator : AbstractValidator<Command>
        {
            ...
            RuleFor(c => c.Name)
                ...
                {
                    var iconGroup = await dbContext.IconGroups
                        .FirstOrDefaultAsync(g => g.PublicId == command.PublicId, cancellation);

                    if (iconGroup == null) return true;

                    return !await dbContext.IconGroups.AnyAsync(g =>
                        g.IconCategory.PublicId == command.IconCategoryPublicId &&
                        g.Name == name &&
                        g.PublicId != command.PublicId, cancellation);
                })
            ...
            RuleFor(c => c.ImagePublicId)
                ...
                {
                    if (!id.HasValue) return true; // không cập nhật ảnh
                    if (id.Value == Guid.Empty) return true; // xóa ảnh
        ...
        public async Task<ApiResult<IconGroupDto>> Handle(Command request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                iconGroup.IconCategoryId = iconCategory.Id;
                iconGroup.Name = request.Name;
                iconGroup.Description = request.Description;
                iconGroup.DisplayOrder = request.DisplayOrder;
                iconGroup.ThuTu = request.ThuTu ?? request.DisplayOrder;
                iconGroup.IsActive = request.IsActive;
                iconGroup.UpdatedAt = DateTime.UtcNow;

                if (request.ImagePublicId.HasValue)
                {
                    if (request.ImagePublicId.Value == Guid.Empty)
                    {
                        iconGroup.ImageUrl = null;
                        iconGroup.ImagePublicId = null;
                    }
                    else
                    {
                        ...
                        if (imageFile != null)
                        {
                            iconGroup.ImageUrl = $"/api/files/{imageFile.PublicId}";
                            iconGroup.ImagePublicId = request.ImagePublicId.Value;
                        }
                    }
                }
                ...
                var result = new IconGroupDto
                {
                    ...
                    TotalIcons = iconGroup.Icons.Count(i => i.IsActive)
                };
                ...
                return new ApiResult<IconGroupDto>
                {
                    Success = true,
                    Data = result,
                    Message = "Cập nhật nhóm icon thành công"
                };
            }
            catch (Exception ex)
            {
                ...
                {
                    Success = false,
                    Message = $"Lỗi cập nhật nhóm icon: {ex.Message}"
                };
            }
        }
    }
