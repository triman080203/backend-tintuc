    ...
    public class Validator : AbstractValidator<Command>
    {
        public Validator(ZaloMiniAppDbContext dbContext, FileDbContext fileContext)
        {
            ...
            RuleFor(c => c)
                ...
                {
                    // Either IconImageUrl or ImagePublicId must be provided (or neither for update)
                    if (!string.IsNullOrEmpty(command.IconImageUrl) && command.ImagePublicId.HasValue)
                        return false; // Both provided - not allowed

                    // Allow empty string as explicit deletion signal
                    if (command.IconImageUrl != null && command.IconImageUrl == string.Empty)
                        return true;

                    // If IconImageUrl provided, validate it's a valid URL
                    if (!string.IsNullOrEmpty(command.IconImageUrl))
                        return Uri.TryCreate(command.IconImageUrl, UriKind.RelativeOrAbsolute, out _);

                    // If ImagePublicId provided, validate file exists
                    if (command.ImagePublicId.HasValue)
                        return await fileContext.Files.AnyAsync(f => f.PublicId == command.ImagePublicId, cancellation);

                    return true; // Neither provided is okay for update (keeps existing)
                })
                .WithMessage("Không thể cung cấp cả IconImageUrl và ImagePublicId cùng lúc");
            ...
            RuleFor(c => c.WebLink)
                ...
                {
                    if (command.IconType == "web" && !string.IsNullOrWhiteSpace(webLink))
                        return Uri.TryCreate(webLink, UriKind.Absolute, out _);
                    return true;
                })
                .WithMessage("WebLink phải là URL hợp lệ khi có giá trị và IconType là 'web'");
        ...
        public async Task<ApiResult<IconDto>> Handle(Command request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (request.ImagePublicId.HasValue)
                {
                    ...
                    if (imageFile != null)
                    {
                        icon.IconImageUrl = $"/api/files/{imageFile.PublicId}";
                        icon.ImagePublicId = request.ImagePublicId;
                    }
                }
                else if (request.IconImageUrl != null && request.IconImageUrl == string.Empty)
                {
                    // Explicit deletion
                    icon.IconImageUrl = null;
                    icon.ImagePublicId = null;
                }
                else if (!string.IsNullOrEmpty(request.IconImageUrl))
                {
                    // Backward compatibility - use provided URL
                    icon.IconImageUrl = request.IconImageUrl;
                    icon.ImagePublicId = null; // Clear the ImagePublicId if using old pattern
                }
                // If neither provided, keep existing values

                icon.IconCategoryId = iconCategory?.Id;
                icon.IconGroupId = iconGroup?.Id;
                icon.Name = request.Name;
                icon.Description = request.Description;
                icon.IconType = request.IconType;
                icon.ScreenParams = request.ScreenParams;
                icon.WebLink = request.WebLink;
                icon.LinkAndroid = request.LinkAndroid;
                icon.LinkIOS = request.LinkIOS;
                icon.DisplayOrder = request.DisplayOrder;
                icon.ThuTu = request.ThuTu ?? request.DisplayOrder;
            ...
            catch (Exception ex)
            {
                ...
                {
                    Success = false,
                    Message = $"Lỗi cập nhật icon: {ex.Message}"
                };
            }
        }
    }
