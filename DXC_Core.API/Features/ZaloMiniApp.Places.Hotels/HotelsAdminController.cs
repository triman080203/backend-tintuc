using Microsoft.AspNetCore.Mvc;
using MediatR;
using Microsoft.AspNetCore.Authorization;
using DXC_Core.API.Shared.Contracts;
using Microsoft.AspNetCore.Http;
using System.Collections.Generic;

namespace DXC_Core.API.Features.ZaloMiniApp.Places.Hotels;

// Controller quản trị khách sạn - dành cho Admin Dashboard
[ApiController]
[Route("api/zalo-mini-app/admin/hotels")]
[Tags("ZaloMiniAppHotelsAdmin")]
[Authorize] // Chỉ admin mới được truy cập tất cả endpoints
public class HotelsAdminController : ControllerBase
{
    private readonly ISender _sender;

    public HotelsAdminController(ISender sender)
    {
        _sender = sender;
    }

    // Lấy danh sách khách sạn với phân trang và filter (Admin)
// query: Tham số tìm kiếm và phân trang
// returns: Danh sách khách sạn với thông tin đầy đủ
    [HttpGet]
    [ProducesResponseType(typeof(PagedResult<HotelWithImagesDto>), StatusCodes.Status200OK)]
    public async Task<IActionResult> GetHotels([FromQuery] GetHotels.Query query)
    {
        var result = await _sender.Send(query);
        return Ok(result);
    }

    // Lấy thông tin chi tiết khách sạn theo ID (Admin)
// id: ID khách sạn
// returns: Thông tin chi tiết khách sạn
    [HttpGet("{publicId:guid}")]
    [ProducesResponseType(typeof(ApiResult<HotelWithImagesDto>), StatusCodes.Status200OK)]
    public async Task<IActionResult> GetHotelById(Guid publicId)
    {
        var query = new GetHotelById.Query { PublicId = publicId };
        var result = await _sender.Send(query);
        return Ok(result);
    }

    // Tạo khách sạn mới (Admin)
// command: Thông tin khách sạn mới
// returns: ID khách sạn đã tạo
    [HttpPost("create")]
    [Authorize] // Chỉ Admin và HotelManager mới được tạo
    [ProducesResponseType(typeof(ApiResult<HotelWithImagesDto>), StatusCodes.Status200OK)]
    public async Task<IActionResult> CreateHotel([FromBody] CreateHotel.Command command)
    {
        var result = await _sender.Send(command);
        return Ok(result);
    }

    // Cập nhật thông tin khách sạn (Admin)
// command: Thông tin cập nhật
// returns: Kết quả cập nhật
    [HttpPost("update")]
    [Authorize] // Chỉ Admin và HotelManager mới được cập nhật
    [ProducesResponseType(typeof(ApiResult<HotelWithImagesDto>), StatusCodes.Status200OK)]
    public async Task<IActionResult> UpdateHotel([FromBody] UpdateHotel.Command command)
    {
        var result = await _sender.Send(command);
        return Ok(result);
    }

    // Xóa khách sạn (Admin)
// command: Thông tin xóa khách sạn
// returns: Kết quả xóa
    [HttpPost("delete")]
    [Authorize(Roles = "admin")] // Chỉ Admin mới được xóa
    [ProducesResponseType(typeof(ApiResult), StatusCodes.Status200OK)]
    public async Task<IActionResult> DeleteHotel([FromBody] DeleteHotel.Command command)
    {
        var result = await _sender.Send(command);
        return Ok(result);
    }

    
}