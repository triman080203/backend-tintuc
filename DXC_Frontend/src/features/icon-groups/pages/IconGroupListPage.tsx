...
import { IconGroupTable } from '../components/IconGroupTable'
import { useIconGroups, useDeleteIconGroup } from '../hooks/useIconGroups'
import type { IconGroupTableRow } from '../types'

export const IconGroupListPage = () => {
  ...
  // ====== DATA FETCHING ======
  const { data, isLoading } = useIconGroups({
    ...getPaginationParams(),
  ...
  const handleDelete = (id: string) => {
    if (confirm('Bạn có chắc muốn xóa nhóm icon này?')) {
      deleteQuery.mutate(id)
    }
  }

  // ====== RENDER ======
  const items = (data?.data || []) as IconGroupTableRow[]
  ...
  return (
    <>
      ...
      <ListPageLayout
        ...
        breadcrumbItems={[
          { label: 'Quản lý nhóm icon', current: true }
        ]}
        actionBarContent={
          <>
            {/* Create Button */}
            <Button
              variant="ghost"
              size="sm"
              onClick={() => navigate('/icon-groups/create')}
              className="gap-2"
            >
      ...
      <Dialog open={isSearchOpen} onOpenChange={handleOpenSearch}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Tìm kiếm nhóm icon</DialogTitle>
          </DialogHeader>
