...
import { useHomestayImageUpload } from '../hooks/useHomestayImageUpload'
import { buildImageUrl } from '../utils/imageUrl'
import type { HomestayImageDto } from '@/api/models'

interface HomestayImageUploaderProps {
  onImagesUploaded: (publicIds: string[]) => void
  disabled?: boolean
  onUploadingChange?: (isUploading: boolean) => void
  existingImages?: HomestayImageDto[]
}

export function HomestayImageUploader({
  onImagesUploaded,
  disabled = false,
  onUploadingChange,
  existingImages = [],
}: HomestayImageUploaderProps) {
  const { uploadedImages, isUploading, handleUpload, removeImage } = useHomestayImageUpload()
  ...
  }, [uploadedImages, existingImages.length, removedExisting.size, onImagesUploaded])
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {existingImages.filter(img => !removedExisting.has(img.imagePublicId || '')).map((image, index) => {
              ...
              return (
                <div key={`existing-${image.imagePublicId || index}`} className="relative group">
                  {imageUrl ? (
                    <img
                      src={imageUrl}
