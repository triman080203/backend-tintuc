...
import { useOcopProductImageUpload } from '../hooks/useOcopProductImageUpload'
import { buildImageUrl } from '../utils/imageUrl'
import type { OcopProductImageDto } from '@/api/models'

interface OcopProductImageUploaderProps {
  onImagesUploaded: (publicIds: string[]) => void
  disabled?: boolean
  onUploadingChange?: (isUploading: boolean) => void
  existingImages?: OcopProductImageDto[]
}

export const OcopProductImageUploader = ({ 
  onImagesUploaded, 
  disabled = false,
  onUploadingChange,
  existingImages = []
}: OcopProductImageUploaderProps) => {
  const {
    uploadedImages,
    isUploading,
    handleUpload,
    removeImage,
  } = useOcopProductImageUpload()
  ...
  // Sync all images (existing + new, excluding removed) to parent
  useEffect(() => {
    const filteredExisting = existingImages
      .filter((img) => !removedExisting.has(img.publicId || ''))
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        />
        <label htmlFor="ocop-product-image-input" className="flex flex-col items-center justify-center cursor-pointer gap-2">
      ...
      {existingImages.length > 0 && (
        <div className="space-y-2">
          <p className="text-sm font-medium text-gray-700">Ảnh hiện tại ({existingImages.filter(img => !removedExisting.has(img.publicId || '')).length})</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {existingImages.filter(img => !removedExisting.has(img.publicId || '')).map((image, index) => {
      ...
      {uploadedImages.length > 0 && (
        <div className="space-y-2">
          <p className="text-sm font-medium text-gray-700">Ảnh mới thêm ({uploadedImages.length})</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
