...
import { useRestaurantImageUpload } from '../hooks/useRestaurantImageUpload'
import { buildImageUrl } from '../utils/imageUrl'
import type { RestaurantImageDto } from '@/api/models'

interface RestaurantImageUploaderProps {
  onImagesUploaded: (publicIds: string[]) => void
  disabled?: boolean
  onUploadingChange?: (isUploading: boolean) => void
  existingImages?: RestaurantImageDto[]
}
...
export function RestaurantImageUploader({
  ...
  const { uploadedImages, isUploading, handleUpload, removeImage } = useRestaurantImageUpload()
              ...
              return (
                <div key={`existing-${image.imagePublicId || index}`} className="relative group">
                  {imageUrl ? (
                    <img
                      src={imageUrl}
                      alt={image.caption || `Restaurant image ${index + 1}`}
