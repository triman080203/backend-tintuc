...
import { useFeedbackProcessingByDepartment } from '../hooks'
...
const FeedbackProcessingPage = () => {
  ...
  // Auto-refetch data when route changes to ensure fresh data
  useEffect(() => {
    // Invalidate all feedback-related queries when navigating to this page
    queryClient.invalidateQueries({ queryKey: ['feedback-processing'] })
    queryClient.invalidateQueries({ queryKey: ['feedback-list'] })
  }, [location.pathname, queryClient])

  const { data: currentUser, isLoading: userLoading } = useCurrentUser()
  const { data, isLoading: dataLoading } = useFeedbackProcessingByDepartment(
    {
      ...getPaginationParams(),
  ...
  if (userLoading) {
    return (
      <ListPageLayout
        ...
        breadcrumbItems={[{ label: 'Xử lý phản ánh', current: true }]}
        actionBarContent={<></>}
      >
      ...
      >
        <div className="flex flex-col items-center justify-center py-12 bg-muted/50 rounded-lg border border-dashed">
          ...
          <p className="text-sm text-muted-foreground max-w-md text-center">
            Để xem danh sách phản ánh xử lý, hãy liên hệ quản trị viên để được gán vào một phòng ban.
          </p>
        </div>
      </ListPageLayout>
    )
    ...
    <>
      ...
      >
        <FeedbackTable
          ...
          detailPath="/feedback/processing"
        />
      </ListPageLayout>
