...
import { useFeedbackPublicByDepartment } from '../hooks'
...
const FeedbackPublicPage = () => {
  ...
  const { data, isLoading: dataLoading } = useFeedbackPublicByDepartment(
    {
      ...getPaginationParams(),
  ...
  if (userLoading) {
    return (
      <ListPageLayout
        ...
        breadcrumbItems={[{ label: 'Phản ánh hoàn thành', current: true }]}
        actionBarContent={<></>}
      >
      ...
      >
        <div className="flex flex-col items-center justify-center py-12 bg-muted/50 rounded-lg border border-dashed">
          ...
          <p className="text-sm text-muted-foreground max-w-md text-center">
            Để xem danh sách phản ánh hoàn thành, hãy liên hệ quản trị viên để được gán vào một phòng ban.
          </p>
        </div>
      </ListPageLayout>
    )
